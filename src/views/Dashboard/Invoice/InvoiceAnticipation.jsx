...
class InvoiceAnticipation extends React.Component {
  constructor(props) {
    super(props)
      ...
      .then(function (response) {
        ...
        if (response.status === Constants.CODE_SUCCESS) {
          if (response.data.invoices.length === 0) {
            ...
            self.setState({
              tableData: response.data.invoices.filter(function (prop) {
                return prop.status === 1
              })
            })
          }
        } else {
  ...
  handleClickReview(status) {
    ...
    var newDatePayment = this.state.invoiceSelected.newDatePayment.split("/")[1] + '-' +
                         this.state.invoiceSelected.newDatePayment.split("/")[0] + '-' +
                         this.state.invoiceSelected.newDatePayment.split("/")[2]

    let data = {
      invoice: {
        status: status,
        anticipation: true,
        newDatePayment: newDatePayment
      }
    }
    ...
    axios.put(Constants.ROUTE_INVOICE_UPDATE + this.state.invoiceSelected.number, data, config)
      .then(function (response) {
        ...
        if (response.status === Constants.CODE_SUCCESS) {
          self.successAlert('Sucesso', 'Solicitação de antecipação alterada com sucesso.')
          self.handleClick()
        } else {
          self.errorAlert('Não conseguimos processar sua requisição :(', 'Desculpe, favor tentar novamente.')
        }
  ...
  handleChangeDatePayment(e) {
    if (e.format !== undefined) {
      ...
      invoiceSelected.newDatePayment = datePayment
            ...
            <Modal isOpen={this.state.modalUpdate} toggle={this.toggleModalUpdate} size="lg">
              ...
              <ModalBody>
                <div>
                  <Table>
                    <thead className="tableHeaderEditAcc">
                      <tr>
                        ...
                        <th className="text-left">Nova Data de Pagamento</th>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td className="text-left"> {this.state.invoiceSelected === null ? '' : this.state.invoiceSelected.datePayment} </td>
                        <td className="text-left">
                          <Datetime
                            ...
                            value={this.state.invoiceSelected.newDatePayment}
                          />
                        </td>
                  ...
                  <Button color="info" onClick={() => this.handleClickReview(2)}>Aprovar</Button>
                  <Button color="error" onClick={() => this.handleClickReview(3)}>Rejeitar</Button>
                </div>
              </ModalBody>
            </Modal>
                ...
                <CardBody>
                  <ReactTable
                    ...
                    data={
                      this.state.tableData.map((prop, key) => {
                        return ({
                          ...
                          newDatePayment: prop.newDatePayment !== undefined ? moment(prop.newDatePayment).format('DD/MM/YYYY') : '',
                    ...
                    columns={[
                      ...
                      {
                        Header: 'Nova Data de Pagamento',
                        accessor: "newDatePayment",
                        maxWidth: 120
                      },
                      {
                        Header: "Descrição",
...
export default InvoiceAnticipation
